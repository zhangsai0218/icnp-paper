 ...
 * Copyright (C) 2016 Jiliang Zhang
 *
...
static void *rvmalloc(unsigned long size)
{
	...
	mem = vmalloc_32(size);
	// mem =(void*)__get_free_pages(GFP_KERNEL,8);
	// printk("Ring Buffer Address(0x%p),vmalloc address (0x%p).\n", mem,mem1);
	// printk("Ring Buffer PAGE(0x%p),vmalloc PAGE (0x%p).\n", vmalloc_to_page(mem),vmalloc_to_page(mem1));
...
static void rvfree(void *mem, unsigned long size)
{
	...
	if (!mem)
		return;
	// free_pages((unsigned long) mem,8);
	adr = (unsigned long)mem;
		...
		size_t count, loff_t *pos, int dir_to_dev)
{
	...
	printk(KERN_INFO"%lld\n",(long long)(ktime_to_ns(ktime_sub(end, start))));
	...
	int head, char __user *buf)
{
	...
	BUG_ON(!engine);
	BUG_ON(!buf);
	char *test_buf0=__get_free_pages(GFP_DMA,8);
	char *test_buf1=__get_free_pages(GFP_DMA,8);
	rx_buffer = engine->rx_buffer;
	memcpy(test_buf0,rx_buffer,RX_BUF_SIZE);
	start = ktime_get();
	memcpy(test_buf1,test_buf0,RX_BUF_SIZE/2);
	end = ktime_get();
	free_pages((unsigned long) test_buf1, 8);
	free_pages((unsigned long) test_buf0, 8);
	...
	dbg_tfr("copy %d bytes starts from %p ", pkt_length,
		&rx_buffer[head * RX_BUF_BLOCK]);
	rc = copy_to_user(&buf[0], &rx_buffer[head * RX_BUF_BLOCK],
		pkt_length);	
...
static int xdma_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	//rc = remap_vmalloc_range(vma, (void*)engine->rx_buffer, 0);
	do{
		page = vmalloc_to_page(kaddr);
		rc = vm_insert_page(vma, uaddr, page);
		//printk(KERN_INFO"rc=%i\n,uaddr=%lu,kaddr=%p,vsize=%lu",rc,uaddr,kaddr,vsize);
		if (rc)	return rc;
		uaddr += PAGE_SIZE;
		kaddr += PAGE_SIZE;
		vsize -= PAGE_SIZE;
		...
		size_t count, loff_t *pos)
{
	...
	if (!engine->dir_to_dev && engine->rx_buffer &&
		...
		rc = char_sgdma_read_cyclic(file, buf);
	} else {
		rc = char_sgdma_read_write(file, buf, count, pos, 0);
	}
	end = ktime_get();
